...
if(isset($_GET['ajax'])){
    ...
    if ($page!=1){
        $data[0]='<div style="text-align: center;padding: 5px 0;color: #0e90d2;"><a href="'.$page_url.'"> 第 '.$page.' 页⬇</a></div>';
    }
    while($row = $dsql->GetArray("list")) {
        ...
        $html_str.= ' <a target="_blank" rel="noopener" href="'.$row['arcurl'].'" class="">'.$row['title'].'</a>';
        $html_str.= '</h3>';
        ...
        if (isset($_GET['listtype'])&& $typeid==0){
            switch ($_GET['listtype']){
                case 'img':
                    $html_str.= '<a target="_blank" rel="noopener" href="'.$row['typeurl'] .'" class="am-icon-folder-o"> '. $row['typename'] .' · </a>';
